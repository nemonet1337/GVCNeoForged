...
public class RenderGVCR2_BOSS_MER03K extends RenderLiving<EntityGVCR2_BOSS_MER03K>
{
	private static final ResourceLocation boatTextures = new ResourceLocation("gvcr2:textures/mob/model/robo/MER03K.png");
    private static final IModelCustom tankk = AdvancedModelLoader.loadModel(new ResourceLocation("gvcr2:textures/mob/model/robo/MER03K.mqo"));
    
    private static final ResourceLocation merg01 = new ResourceLocation("gvcr2:textures/mob/model/robo/merg03.png");
    private static final IModelCustom merg01m = AdvancedModelLoader.loadModel(new ResourceLocation("gvcr2:textures/mob/model/robo/merg03.mqo"));
    ...
    public RenderGVCR2_BOSS_MER03K(RenderManager renderManagerIn)
    {
     ...
     */
    public void doRender(EntityGVCR2_BOSS_MER03K entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
		...
		GL11.glRotatef(180.0F - entity.rotationYawHead, 0.0F, 1.0F, 0.0F);//GL11.glRotatef(180.0F - entityYaw, 0.0F, 1.0F, 0.0F);
		{
    ...
    private void renderlegs(EntityGVCR2_BOSS_MER03K entity, float limbSwing, float limbSwingAmount, float partialTicks){
    ...
    private void renderleg_left(EntityGVCR2_BOSS_MER03K entity, float limbSwing, float limbSwingAmount, float partialTicks){
    ...
    private void renderleg_right(EntityGVCR2_BOSS_MER03K entity, float limbSwing, float limbSwingAmount, float partialTicks){
    	float Ax = MathHelper.cos(limbSwing * 0.6662F + (float)Math.PI) * limbSwingAmount;
    	GL11.glPushMatrix();//glstart
    	GL11.glTranslatef(- kneex, kneey, kneez);
    	GL11.glRotatef(-10, 0.0F, 0.0F, 1.0F);
    ...
    private void renderbody(EntityGVCR2_BOSS_MER03K entity, float limbSwing, float limbSwingAmount, float partialTicks){
    	GL11.glTranslatef(bodyx, bodyy, bodyz);
    ...
    private void renderarm_left(EntityGVCR2_BOSS_MER03K entity, float limbSwing, float limbSwingAmount, float partialTicks){
		{
			float elrote_x = -56;//-30
			float elrote_y = 0;//-30
    ...
    private void renderarm_right(EntityGVCR2_BOSS_MER03K entity, float limbSwing, float limbSwingAmount, float partialTicks){
    	...
    	
    	{
			...
			GL11.glTranslatef(handx, -handy, -handz);
			{
				...
				/*if(entity.getClient_cooltime(entity) > 60 && !(entity.deathTime > 0)) {
					GL11.glTranslatef(0, 0, 2.0F);
					float si = 100 - entity.getClient_cooltime(entity);
					float size = si * 0.01F + 1;
					GL11.glScalef(size, size, size);
					GL11.glRotatef(entity.getClient_cooltime(entity) *10, 0.0F, 0.0F, 1.0F);
					//model.renderPart("beam");
					{
						GL11.glPushMatrix();//glstart
    ...
    private void renderarm_rightgun(EntityGVCR2_BOSS_MER03K entity, float limbSwing, float limbSwingAmount, float partialTicks){
   
    }
    
    private void renderarm_leftgun(EntityGVCR2_BOSS_MER03K entity, float limbSwing, float limbSwingAmount, float partialTicks){
        
        }
    private void renderarm_subgun(EntityGVCR2_BOSS_MER03K entity, float limbSwing, float limbSwingAmount, float partialTicks){
    		
        }
    
    private void render_light(EntityGVCR2_BOSS_MER03K entity, String name){
    	GL11.glPushMatrix();//glstart
    	GL11.glDisable(GL11.GL_LIGHT1);
		GL11.glDisable(GL11.GL_LIGHTING);
    	int i = 15728880;
	...
	public float F6(EntityLivingBase entity, float partialTicks){
 	...
 	public void BoostRender(float bx, float by, float bz, String re, EntityGVCR2_BOSS_MER03K entity, float partialTicks){
 		boolean boost = false;
     ...
     */
    protected ResourceLocation getEntityTexture(EntityGVCR2_BOSS_MER03K entity)
    {
    	return boatTextures;
    }
    
    protected void renderLivingAt(EntityGVCR2_BOSS_MER03K entityLivingBaseIn, double x, double y, double z)
    {
        GlStateManager.translate((float)x, (float)y, (float)z);
    }
    protected float handleRotationFloat(EntityGVCR2_BOSS_MER03K livingBase, float partialTicks)
    {
