...
public class RenderGVCR2_GE_SE_KUKRI extends RenderLiving<EntityGVCR2_GE_SE_KUKRI> //extends RenderLiving<EntityDoll>
{
	private static final ResourceLocation boatTextures = new ResourceLocation("gvcr2:textures/mob/model/biped/kukri.png");
    private static final IModelCustom tankk = AdvancedModelLoader.loadModel(new ResourceLocation("gvcr2:textures/mob/model/biped/kukri.mqo"));
    
    public RenderGVCR2_GE_SE_KUKRI(RenderManager renderManagerIn)
    {
    ...
    public void rendermodel(EntityGVCR2_GE_SE_KUKRI entity, String model) {
     ...
     */
    protected ResourceLocation getEntityTexture(EntityGVCR2_GE_SE_KUKRI entity)
    {
     ...
     */
    float iii;
    public void doRender(EntityGVCR2_GE_SE_KUKRI entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
    ...
    private void renderlegs(EntityGVCR2_GE_SE_KUKRI entity, float limbSwing, float limbSwingAmount, float partialTicks){
    	GL11.glPushMatrix();//glstart
    	{
    ...
    private void renderbody(EntityGVCR2_GE_SE_KUKRI entity, float limbSwing, float limbSwingAmount, float partialTicks){
    	if(entity.deathTime > 0){
    	...
    	GL11.glPushMatrix();//glstart
    	{
			...
			if(entity.getattacktask() && !(entity.deathTime > 0)) {
				if(entity.getAIType3() == 1) {
					if(entity.cooltime < 30) {
						arm_l_rotex = -60;
						arm_l_rotez = 60 - entity.cooltime*4;
					}
				}else {
					float sw = entity.swingProgressInt * 30;
					if(entity.swingProgressInt > 0) {
						arm_l_rotex = -120 + sw;
					}else {
		...
		GL11.glPushMatrix();//glstart
		{
			...
			if(entity.getattacktask() && !(entity.deathTime > 0)) {
				...
				if(entity.swingProgressInt > 0) {
					...
					arm_r_rotex = Ax * (180F / (float)Math.PI) - 30;
				}
			}else if(entity.isRiding()) {
				arm_r_rotex = -30;
			}
    ...
    protected void renderLivingAt(EntityGVCR2_GE_SE_KUKRI entityLivingBaseIn, double x, double y, double z)
    {
        GlStateManager.translate((float)x, (float)y, (float)z);
    }
    protected float handleRotationFloat(EntityGVCR2_GE_SE_KUKRI livingBase, float partialTicks)
    {
