...
public class RenderGVCR2_AIR_F18 extends RenderLiving<EntityGVCR2_AIR_F18>
{
    private static final ResourceLocation boatTextures = new ResourceLocation("gvcr2:textures/mob/model/aircraft/f18.png");
    private static final IModelCustom tankk = AdvancedModelLoader.loadModel(new ResourceLocation("gvcr2:textures/mob/model/aircraft/f18.mqo"));
    /** instance of ModelBoat for rendering */

    public RenderGVCR2_AIR_F18(RenderManager renderManagerIn)
    {
     ...
     */
    float iii;
    public void doRender(EntityGVCR2_AIR_F18 entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
		...
		if(entity.getControllingPassenger() != null && entity.getControllingPassenger() == Minecraft.getMinecraft().player && entity.getArmID_R() == 2){
			RenderBomber_Target.can(entity, 2.5F + 2.5F);
		...
		if(entity.getControllingPassenger() != null && entity.getControllingPassenger() == Minecraft.getMinecraft().player){
			...
			if(Minecraft.getMinecraft().gameSettings.thirdPersonView == 0 && right && entity.getArmID_R() == 2){
				flag = true;
			}
		}
		...
		if(!flag) {
			...
			tankk.renderPart("wing_l");
			tankk.renderPart("wing_r");
			tankk.renderPart("weapon_aim");
			
			{
				RenderExhaust.can(entity, 0.6F, 1.8F, -8.5F, 1.0F, true, boatTextures);
				RenderExhaust.can(entity, -0.6F, 1.8F, -8.5F, 1.0F, true, boatTextures);
			}
			...
			}
			{
				...
				else if(entity.getArmID_R() == 2) {
					tankk.renderPart("weapon_b");
				}
				else if(entity.getArmID_R() == 3) {
					tankk.renderPart("weapon_asm");
				}
				else {
					tankk.renderPart("weapon_aam");
				}
			}
     ...
     private void renderAngle(EntityGVCR2_AIR_F18 p_76986_1_, int i){
		{
     ...
     */
    protected ResourceLocation getEntityTexture(EntityGVCR2_AIR_F18 entity)
    {
    	{
			return boatTextures;
		}
    ...
    protected void renderLivingAt(EntityGVCR2_AIR_F18 entityLivingBaseIn, double x, double y, double z)
    {
        GlStateManager.translate((float)x, (float)y, (float)z);
    }
    protected float handleRotationFloat(EntityGVCR2_AIR_F18 livingBase, float partialTicks)
    {
