...
public class RenderGVCR2_BOSS_MER08_HFB extends RenderLiving<EntityGVCR2_BOSS_MER08_HFB>
{
    ...
    private static final ResourceLocation merg01sw = new ResourceLocation("gvcr2:textures/mob/model/robo/merg08hfb.png");
    private static final IModelCustom merg01swm = AdvancedModelLoader.loadModel(new ResourceLocation("gvcr2:textures/mob/model/robo/merg08hfb.mqo"));
    ...
    public RenderGVCR2_BOSS_MER08_HFB(RenderManager renderManagerIn)
    {
     ...
     */
    public void doRender(EntityGVCR2_BOSS_MER08_HFB entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
    ...
    private void renderlegs(EntityGVCR2_BOSS_MER08_HFB entity, float limbSwing, float limbSwingAmount, float partialTicks){
    ...
    private void renderleg_left(EntityGVCR2_BOSS_MER08_HFB entity, float limbSwing, float limbSwingAmount, float partialTicks){
    ...
    private void renderleg_right(EntityGVCR2_BOSS_MER08_HFB entity, float limbSwing, float limbSwingAmount, float partialTicks){
    	float Ax = MathHelper.cos(limbSwing * 0.6662F + (float)Math.PI) * limbSwingAmount;
    	GL11.glPushMatrix();//glstart
    	GL11.glTranslatef(- kneex, kneey, kneez);
    	GL11.glRotatef(-10, 0.0F, 0.0F, 1.0F);
    ...
    private void renderbody(EntityGVCR2_BOSS_MER08_HFB entity, float limbSwing, float limbSwingAmount, float partialTicks){
    	GL11.glTranslatef(bodyx, bodyy, bodyz);
    	...
    	if(entity.cooltime4 < 40  && entity.startuptime > 100){
    		if(entity.cooltime4 < 18){
    			
    		}else if(entity.cooltime4 >= 18 && entity.cooltime4 < 30){
    			GL11.glRotatef(88F , 0.0F, 1.0F, 0.0F);
    		}
    	}
    ...
    private void renderarm_left(EntityGVCR2_BOSS_MER08_HFB entity, float limbSwing, float limbSwingAmount, float partialTicks){
    	{
    ...
    private void renderarm_right(EntityGVCR2_BOSS_MER08_HFB entity, float limbSwing, float limbSwingAmount, float partialTicks){
    	ResourceLocation tex = merg01sw;
		IModelCustom model = merg01swm;
    	float elrote = 0F;//10F
    	float armrote = -30F;//-100F
    	float handrote = 0;//0
    	...
    	
    	{
			...
			if(entity.cooltime4 < 40  && entity.startuptime > 100){
				if(entity.cooltime4 < 18){
					GL11.glTranslatef(-elbowx, elbowy, elbowz);
					GL11.glRotatef(-90, 1.0F, 1.0F, 0.0F);
					GL11.glTranslatef(elbowx, - elbowy, - elbowz);
	    		}else if(entity.cooltime4 >= 18 && entity.cooltime4 < 30){
	    			GL11.glTranslatef(-elbowx, elbowy, elbowz);
    ...
    private void renderarm_rightgun(EntityGVCR2_BOSS_MER08_HFB entity, float limbSwing, float limbSwingAmount, float partialTicks){
   
    }
    
    private void renderarm_leftgun(EntityGVCR2_BOSS_MER08_HFB entity, float limbSwing, float limbSwingAmount, float partialTicks){
        
        }
    private void renderarm_subgun(EntityGVCR2_BOSS_MER08_HFB entity, float limbSwing, float limbSwingAmount, float partialTicks){
    		
        }
    
    private void render_light(EntityGVCR2_BOSS_MER08_HFB entity, String name){
    	GL11.glPushMatrix();//glstart
	...
	public float F6(EntityLivingBase entity, float partialTicks){
 	...
 	public void BoostRender(float bx, float by, float bz, String re, EntityGVCR2_BOSS_MER08_HFB entity, float partialTicks){
 		boolean boost = false;
     ...
     */
    protected ResourceLocation getEntityTexture(EntityGVCR2_BOSS_MER08_HFB entity)
    {
    	return boatTextures;
    }
    
    protected void renderLivingAt(EntityGVCR2_BOSS_MER08_HFB entityLivingBaseIn, double x, double y, double z)
    {
        GlStateManager.translate((float)x, (float)y, (float)z);
    }
    protected float handleRotationFloat(EntityGVCR2_BOSS_MER08_HFB livingBase, float partialTicks)
    {
