...
public class EntityGVCR2_BOSS_MER08_SN extends EntityGVCR2_BOSSBase {
	
	public EntityGVCR2_BOSS_MER08_SN(World worldIn) {
		super(worldIn);
    	...
    	if(this.startuptime < 100) {
    		...
    		if(entity != null){
    			if(entity instanceof IMob){
    	        	...
    	        	par2 = par2 * AI_Damage.newAntiBullet(this, entity2, par2, 0.75F, 1.0F, 1.0F, 1.0F);
    	        	if(entity2 instanceof EntityB_BulletFire)
    	    		{
    	        		par2 = 0;
    	            }
    	        	if(!(this.getHealth() > this.getMaxHealth()/2) && this.getAIType2() != 0) {
    	        		par2 = par2 * 0.25F;
    	        	}
    				if(par2 <= 0.0) {
    				...
    				/*if(this.cooltime < 60 && this.getHealth() > this.getMaxHealth()/2){
    					par2 = 0;
    				}*/
	...
	public void onUpdate()
    {
        ...
        if(FMLCommonHandler.instance().getMinecraftServerInstance() != null) {
			for (EntityPlayerMP playermp : FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList().getPlayers()) {
				GVCLPacketHandler.INSTANCE2.sendTo(new GVCLClientMessageKeyPressed(2001, this.getEntityId(), cooltime), playermp);
				GVCLPacketHandler.INSTANCE2.sendTo(new GVCLClientMessageKeyPressed(2014, this.getEntityId(), counter1), playermp);
			}
		}
    }
	...
	protected void onDeathUpdate() {
		...
		if(!this.world.isRemote && this.deathTime == 100) {
			GVCExplosionBase.ExplosionKai(this, this, this.posX + 0, this.posY + 0, this.posZ + 0, 2, false,  false);
			{
				...
				Chest3.setInventorySlotContents(13, new ItemStack(mod_GVCR2.gvcr2_gun_railgun, 1, 0));
	...
	public void onLivingUpdate() {
		...
		if (this.world.isRemote)spawn_p();
		
		/*if(this.cooltime > 60){
			float yaw = this.rotationYawHead * (2 * (float) Math.PI / 360);
			double mox = 0;
			double moz = 0;
			mox -= MathHelper.sin(yaw)  * 1.0;
			moz += MathHelper.cos(yaw)  * 1.0;
			mox -= MathHelper.sin(yaw + 1.57F)  * 0.7;
			moz += MathHelper.cos(yaw + 1.57F)  * 0.7;
			this.world.spawnParticle(EnumParticleTypes.CLOUD, 
            		this.posX + mox, this.posY + 1.5, this.posZ + moz, 0, 0, 0);
		}*/
		
		{
			if(AI_EntityWeapon.getRange(this, 80, 40, 20)){
				{
					String model = "gvclib:textures/entity/model/bullet_aaa.mqo";
		    		String tex = "gvclib:textures/entity/model/bullet_beam.png";
		    		String modelf = "gvclib:textures/entity/model/non.mqo";
		    		String texf = "gvclib:textures/entity/model/non.png";
		        	{
		        		...
		        		if(this.counter1){
		        			...
		        			AI_EntityWeapon.Attacktask_new(this, this, 0, 80, 
		        					model, tex, modelf, texf,2, sounds, 
		        					1.57F, 0.8, 1.5, 0.9, 0, 0, this.posX, this.posY, this.posZ,looked,this.rotationYawHead, 10, 10,
		    						20, 16, 1, 0, false, 1, 0.029D, 80, 0);
		...
		{// 1
			...
			
			{
				...
				if(!(entity.getHealth() > entity.getMaxHealth()/2)) {
					...
					if (this.world.isRemote)spawn_p();
					if(entity.aitypetime2 == 1) {
						if(entity.getAIType2() == 1) {
							moy = 1.0;
							mox = 1.5;
						}
						else if(entity.getAIType2() == 2) {
							moy = 1.0;
							moz = 1.5;
						}
						else if(entity.getAIType2() == 3) {
							moy = 1.0;
							mox = -1.5;
						}
						else if(entity.getAIType2() == 4) {
							moy = 1.0;
							moz = -1.5;
						}
						else// if(entity.getAIType2() == 0) 
						{
							mox -= MathHelper.sin(yaw)  * 1;
							moz += MathHelper.cos(yaw)  * 1;
							moy = 0.75;
						}
						
						/*int ran = 20;
						if(entity.getAIType2() == 1) {
							BlockPos bp = this.world.getHeight(new BlockPos(entity.getMoveX()  + ran, entity.getMoveY(), entity.getMoveZ()  + ran));
							int genY = bp.getY();
							if(!entity.world.isRemote)entity.setPositionAndUpdate(entity.getMoveX()  + ran, genY, entity.getMoveZ()  + ran);
						}
						else if(entity.getAIType2() == 2) {
							BlockPos bp = this.world.getHeight(new BlockPos(entity.getMoveX()  - ran, entity.getMoveY(), entity.getMoveZ()  - ran));
							int genY = bp.getY();
							...
							if(!entity.world.isRemote)entity.setPositionAndUpdate(ex, ey, ez);
						}
						else {
						}*/
					}
