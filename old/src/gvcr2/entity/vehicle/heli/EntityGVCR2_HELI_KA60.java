...
public class EntityGVCR2_HELI_KA60 extends EntityMobVehicleBase
{

    public EntityGVCR2_HELI_KA60(World worldIn)
    {
        super(worldIn);
        ...
        this.spawn_item = mod_GVCR2.gvcr2_spawn_heli_ka60;
    ...
    int tasktime = 0;
    private void heli_airborne() {
    	/*if(this.getHealth() <= 0.0F) {
			this.thpower = 0;
			this.throttle = 0;
			this.rotationp = this.rotationPitch = 15;
		}else */
		{
			this.thpower = thmax;
			this.throttle = thmax;
		...
		else if(this.getFTMode() == 32) {
			this.rotationp = this.rotationPitch = 20;
		}
		else {
			this.rotationp = this.rotationPitch = 20;
		}
    	...
    	AI_AirCraftSet.setheli_NEW(this, GVCSoundEvent.sound_heli, 0, this.sp, 0.1F);
    	
    	double d5 = this.getMoveX() - this.posX;
		double d7 = this.getMoveZ() - this.posZ;
		this.rote = -((float) Math.atan2(d5, d7)) * 180.0F / (float) Math.PI;
		if(this.getFTMode() == 30 || this.getFTMode() == 35)rote(this, turnspeed);
		...
		if(this.getFTMode() == 30) {
			if ((ddx < this.getArmID_S() && ddz < this.getArmID_S())) {
				this.setFTMode(31);
			}
		}
		if(this.getFTMode() == 31) {
			...
			if(tasktime > 40) {
				if(tasktime %10 == 0 &&  this.getPassengers().size() > 1) {
					if (this.getPassengers().get(1) != null) {
						if (this.getPassengers().get(1) instanceof EntityLivingBase) {
				...
				/*for(int m = 0; m < this.getPassengers().size(); ++m) {
					if (m != 0 && this.getPassengers().get(m) != null) {
