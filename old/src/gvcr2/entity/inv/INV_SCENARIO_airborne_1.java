...
public class INV_SCENARIO_airborne_1 {
	...
	public void scenario(EntityGVCR2_INV_SCENARIO inv_entity, int flagtimer, boolean flag_xp, boolean flag_xm, boolean flag_zp, boolean flag_zm) {
		...
		if(inv_entity.getHealth() < spawn_start_time && inv_entity.getArmID_A() == 0 && spawn_flag){
			...
			for(int c = 0; c < 3;++c){//for
				...
				{
					if (!world.isRemote) {
						EntityGVCR2_HELI_GHELI car = new EntityGVCR2_HELI_GHELI(world);
						car.setcanDespawn(1);
						car.throttle = car.thmax;
						car.thpower = car.thmax;
						car.setFTMode(30);
						car.setArmID_S(10 + world.rand.nextInt(10));
						car.setMoveT(1);
		...
		//WAVE4
				if(inv_entity.getHealth() < spawn_start_time && inv_entity.getArmID_A() == 3 && spawn_flag){
							...
							{
								if (!world.isRemote) {
									EntityGVCR2_HELI_MI24D car = new EntityGVCR2_HELI_MI24D(world);
									car.setcanDespawn(1);
									car.throttle = car.thmax;
									car.thpower = car.thmax;
									car.setMoveT(1);
					...
					if(!world.isRemote) {
						{
							...
							Chest3.setInventorySlotContents(13, new ItemStack(mod_GVCR2.gvcr2_item_inv_airborne_1, 1, 0));
