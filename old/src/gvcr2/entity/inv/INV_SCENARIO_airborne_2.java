...
public class INV_SCENARIO_airborne_2 {
		...
		if(inv_entity.getHealth() < spawn_start_time && inv_entity.getArmID_A() == 0 && spawn_flag){
			...
			for(int c = 0; c < 3;++c){//for
				...
				if(c == 0) {
					...
					if (!world.isRemote) {
						EntityGVCR2_HELI_R22 car = new EntityGVCR2_HELI_R22(world);
						car.setcanDespawn(1);
					...
					if(!world.isRemote) {
						{
							...
							Chest3.setInventorySlotContents(13, new ItemStack(mod_GVCR2.gvcr2_item_inv_airborne_2, 1, 0));
