...
public class INV_SCENARIO_tank_3 {
	...
	public void scenario(EntityGVCR2_INV_SCENARIO inv_entity, int flagtimer, boolean flag_xp, boolean flag_xm, boolean flag_zp, boolean flag_zm) {
		...
		if(inv_entity.getHealth() < spawn_start_time && inv_entity.getArmID_A() == 2 && spawn_flag){
					...
					for(int c = 0; c < 3;++c){//for
						...
						if(c == 0){
							if (!world.isRemote) {
								EntityGVCR2_TANK_T72 car = new EntityGVCR2_TANK_T72(world);
								car.setArmID_L(0);
								car.setcanDespawn(1);
								car.setMoveT(1);
								car.setMoveX((int)inv_entity.posX + scenario_base.random_int(world, 10));
		...
		//WAVE4
				if(inv_entity.getHealth() < spawn_start_time && inv_entity.getArmID_A() == 3 && spawn_flag){
							...
							for(int c = 0; c < 3;++c){//for
								...
								if(c == 0){
									if (!world.isRemote) {
										EntityGVCR2_TANK_2K22 car = new EntityGVCR2_TANK_2K22(world);
										car.setArmID_L(0);
										car.setcanDespawn(1);
										car.setMoveT(1);
										car.setMoveX((int)inv_entity.posX + scenario_base.random_int(world, 10));
					...
					if(!world.isRemote) {
						{
							...
							Chest3.setInventorySlotContents(13, new ItemStack(mod_GVCR2.gvcr2_item_inv_tank_3, 1, 0));
