...
public class INV_SCENARIO_airborne_4 {
					...
					if(!world.isRemote) {
						{
							...
							Chest3.setInventorySlotContents(13, new ItemStack(mod_GVCR2.gvcr2_spawn_ship_airship, 1, 0));
							Chest3.setInventorySlotContents(1, new ItemStack(mod_GVCR2.gvcr2_item_inv_airborne_4, 1, 0));
		...
		//WAVE1
				if(inv_entity.getHealth() < spawn_start_time && inv_entity.getArmID_A() == 0 && spawn_flag){
					...
					for(int c = 0; c < 3;++c){//for
						...
						{
							spawn_bomber(inv_entity, scenario_base, world, 1, range_x, range_z, xx, zz);
						}
						{
							spawn_bomber(inv_entity, scenario_base, world, 0, range_x, range_z, xx, zz);
						}
					}//for
				}//WAVE1
	}
		...
		//WAVE2
				if(inv_entity.getHealth() < spawn_start_time && inv_entity.getArmID_A() == 1 && spawn_flag){
					...
					for(int c = 0; c < 3;++c){//for
						...
						if(c != 0){
							spawn_fighter(inv_entity, scenario_base, world, 0, range_x, range_z, xx, zz);
						}
					}//for
				}//WAVE2
	}
	...
	public void wave5(EntityGVCR2_INV_SCENARIO inv_entity, INV_SCENARIO scenario_base, int spawn_start_time, boolean spawn_flag, int range_x, int range_z) {
		...
		if(inv_entity.getHealth() < spawn_start_time && inv_entity.getArmID_A() == 4 && spawn_flag){
					...
					for(int c = 0; c < 3;++c){//for
						...
						{
							spawn_fighter(inv_entity, scenario_base, world, 1, range_x, range_z, xx, zz);
						}
					}//for
				}//WAVE5
	}
	...
	public void spawn_fighter(EntityGVCR2_INV_SCENARIO inv_entity, INV_SCENARIO scenario_base, World world, int id, int range_x, int range_z, int xx, int zz) {
		if (!world.isRemote) {
			EntityMobVehicleBase car;
			if(id == 1) {
				car = new EntityGVCR2_AIR_SU27(world);
			}else {
				car = new EntityGVCR2_AIR_YAK23KAI(world);
			}
	...
	public void spawn_bomber(EntityGVCR2_INV_SCENARIO inv_entity, INV_SCENARIO scenario_base, World world, int id, int range_x, int range_z, int xx, int zz) {
		if (!world.isRemote) {
