...
public class INV_SCENARIO_tank_2 {
	...
	public void scenario(EntityGVCR2_INV_SCENARIO inv_entity, int flagtimer, boolean flag_xp, boolean flag_xm, boolean flag_zp, boolean flag_zm) {
		...
		if(inv_entity.getHealth() < spawn_start_time && inv_entity.getArmID_A() == 1 && spawn_flag){
			...
			{
				if (!world.isRemote) {
					EntityGVCR2_TANK_ZSU_23_4 car = new EntityGVCR2_TANK_ZSU_23_4(world);
					car.setArmID_L(0);
					car.setcanDespawn(1);
					car.setMoveT(1);
					car.setMoveX((int)inv_entity.posX + scenario_base.random_int(world, 10));
					car.setMoveY((int)inv_entity.posY);
					car.setMoveZ((int)inv_entity.posZ + scenario_base.random_int(world, 10));
					double x_pos = inv_entity.posX + range_x + 0 +scenario_base.random_int(world, 10);
					double y_pos = inv_entity.posY + 2;
					double z_pos = inv_entity.posZ + range_z  + 0 +scenario_base.random_int(world, 10);
					double d5 = inv_entity.posX - x_pos;
					double d7 = inv_entity.posZ - z_pos;
					float rote = -((float) Math.atan2(d5, d7)) * 180.0F / (float) Math.PI;
					car.setLocationAndAngles(x_pos, y_pos, z_pos, rote, 0F);
		...
		if(inv_entity.getHealth() < spawn_start_time && inv_entity.getArmID_A() == 2 && spawn_flag){
					...
					for(int c = 0; c < 3;++c){//for
						...
						if(c == 0){
							if (!world.isRemote) {
								EntityGVCR2_TANK_T55 car = new EntityGVCR2_TANK_T55(world);
								car.setArmID_L(0);
								car.setcanDespawn(1);
								car.setMoveT(1);
								car.setMoveX((int)inv_entity.posX + scenario_base.random_int(world, 10));
					...
					{
						if (!world.isRemote) {
							EntityGVCR2_TANK_KV2 car = new EntityGVCR2_TANK_KV2(world);
							car.setArmID_L(0);
							car.setcanDespawn(1);
							car.setMoveT(1);
							car.setMoveX((int)inv_entity.posX + scenario_base.random_int(world, 10));
					...
					if(!world.isRemote) {
						{
							...
							Chest3.setInventorySlotContents(13, new ItemStack(mod_GVCR2.gvcr2_item_inv_tank_2, 1, 0));
