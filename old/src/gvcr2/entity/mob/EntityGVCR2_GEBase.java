...
public class EntityGVCR2_GEBase extends EntityGVCLivingBase implements IMob
{
    ...
    public EntityGVCR2_GEBase(World worldIn)
    {
        super(worldIn);
    }
    ...
    int time = 0;
    public void doblock(EntityGVCR2_GEBase entity, double range, Block block) {
		boolean flag = true;
		/*{
			...
			if (llist != null) {
				for (int lj = 0; lj < llist.size(); lj++) {
					...
					if (entity1.canBeCollidedWith()) {
						boolean see = entity.getEntitySenses().canSee(entity1);
						if (entity1 != null && entity1 instanceof EntityPlayer && see) {//target
							flag = false;
							time = 0;
							break;
						}//target
		...
		/*if((this.getattacktask() || entity.targetentity == null)) {
			flag = false;
			time = 0;
		}*/
		/*{
		...
		if(flag && !(this.getattacktask() && entity.targetentity == null)) {
			++time;
			if(!this.world.isRemote && time > 200){
            	this.world.setBlockState(this.getPosition(), block.getDefaultState());
            	this.setDead();
            }
		}*/
	}
    ...
    public boolean CanAttack(Entity entity){
    	if(!(entity instanceof EntityGVCR2_GEBase) && entity instanceof EntityGolem && ((EntityLivingBase) entity).getHealth() > 0.0F){
    		return true;
