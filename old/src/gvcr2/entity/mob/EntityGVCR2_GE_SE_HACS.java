...
public class EntityGVCR2_GE_SE_HACS extends EntityGVCR2_GEBase {

	public EntityGVCR2_GE_SE_HACS(World worldIn) {
		super(worldIn);
	...
	public void setWeapon() {
		this.setItemStackToSlot(EntityEquipmentSlot.MAINHAND, new ItemStack(mod_GVCR2.gvcr2_gun_xm25));
	}
    ...
    {
    	this.entityDropItem(new ItemStack(mod_GVCR2.gvcr2_gun_xm25), 0.0F);
    }
	...
	public void onLivingUpdate() {
		...
		this.motionY = this.motionY * 0.75D;
		...
		
		{
			...
			x1 -= MathHelper.sin(ff -1.57F) * 0.72D;
			z1 += MathHelper.cos(ff -1.57F) * 0.72D;
			x1 -= MathHelper.sin(ff) * -0.4D;
			z1 += MathHelper.cos(ff) * -0.4D;
			this.world.spawnParticle(EnumParticleTypes.CLOUD, this.posX + x1, this.posY + 1.2D, this.posZ + z1, 0.0D, 0.0D, 0.0D, new int[0]);
			this.world.spawnParticle(EnumParticleTypes.CLOUD, this.posX - x1, this.posY + 1.2D, this.posZ - z1, 0.0D, 0.0D, 0.0D, new int[0]);
		}
		...
		{
			{
	    		if(this.cooltime > 60 && this.getRemain_L() > 0)
				{
	    			this.counter1 = true;
	    			this.cooltime = 0;
				}
	    		if(this.counter1 && this.gun_count1 > 10 && this.getRemain_L() > 0){
	    			{
			    		...
			    		SoundEvent sounds = GVCSoundEvent.fire_mg;
								...
								we.power = 10 + this.world.getDifficulty().getDifficultyId();
								we.damegetime = false;
								we.speed = 1.6F;
								we.bure = 3F;
								we.ex = 2;
